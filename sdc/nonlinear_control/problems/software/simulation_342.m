% Week 3: Concept check 4, Question 2
% 2022-03-4
...
for i = 1:length(time)-1
    ...
    u = control(I, omega, omega_rn, omega_dot_rn, sigma, sigma_rn, K, P, Ki, dt, L, clear_flag);
...
function [u] = control(I, omega_bn, omega_rn, omega_dot_rn, ...
    sigma_bn, sigma_rn, K, P, Ki, dt, L, clear_flag)
...
sum  = sum + sigma_br .* dt;
z = K * sum + I * (omega_br - omega_br_o);

% Control law
u = -K * sigma_br - P * omega_br + cross(omega_bn, I * omega_bn) ...
    + I * (omega_dot_rn - cross(omega_bn, omega_rn)) - P * Ki * z + L;
end
